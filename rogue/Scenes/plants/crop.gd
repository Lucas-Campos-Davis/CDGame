...
func _process(delta):		
	if $AnimatedSprite2D.frame == ripe_frame:
		if time_since_last_growth < canvas_modulate.time - ((stage_length + randf_range(0, max_random_length))*ripe_length):
			$AnimatedSprite2D.frame += 1;
			time_since_last_growth = canvas_modulate.time;
	else:
		if time_since_last_growth < canvas_modulate.time - (stage_length + randf_range(0, max_random_length)):
			$AnimatedSprite2D.frame += 1;
			time_since_last_growth = canvas_modulate.time;
